...
class CommandExecutor {
    _execPromise;
    constructor(execPromiseOverride) {
    ...
    async isWaitingForUserInput(ttyPath) {
        let fd;
        try {
    ...
    async retrieveTtyPath(sessionId) {
        try {
            const { stdout } = await this._execPromise(osascriptCommand(scriptForSession(sessionId, 'return (get tty)')));
            return stdout.trim();
        }
        catch (error) {
            throw new Error(`Failed to retrieve TTY path: ${error.message}`);
        }
    }
    async isProcessing(sessionId) {
        try {
