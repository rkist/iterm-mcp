 ...
 */
export function classifySession(jobName, commandLine, foregroundChain) {
  ...
  if (count of windows) is 0 then
    ...
    tell current window
      ...
      tell t to tell current session to return unique id
        ...
        if (!id)
            throw new Error('iTerm2 did not return a session id for the new tab');
        return id;
    }
    /**
     ...
     */
    async foregroundProcesses(ttyPath) {
        const ttyName = basename(ttyPath);
        if (!/^ttys?\d+$/.test(ttyName))
            return [];
        try {
