...
const WRITE_GUARDED = new Set(['remote', 'container']);
...
export default class SessionSelector {
    ...
    autoOpenNewTab;
    sleep;
    constructor(inspector = new SessionInspector(), options = {}) {
    ...
    async resolve(opts) {
        ...
        if (opts.sessionId) {
            ...
            if (!explicit) {
                throw new SessionResolutionError(`iTerm session ${opts.sessionId} not found.\n\n${formatSessionList(sessions, null)}`, sessions);
            }
        ...
        throw new SessionResolutionError(`No local shell session is available to target.\n${hint}\n` +
    ...
    async openNewSession() {
        ...
        for (let i = 0; i < NEW_SESSION_MAX_POLLS; i++) {
            ...
            if (seen && seen.kind === 'local-shell' && !seen.isProcessing)
                break;
            await this.sleep(NEW_SESSION_POLL_MS);
        }
        if (!seen) {
            throw new SessionResolutionError(`Opened a new iTerm tab (${id}) but it did not show up in the session list.`, await this.inspector.listSessions());
        }
        this.pinnedId = seen.id;
        return seen;
    }
    guardWrite(session, forWrite, allowRemote, sessions, explicit) {
        if (!forWrite || allowRemote || !WRITE_GUARDED.has(session.kind))
            return;
        const how = explicit ? 'The requested session' : 'The session this server is pinned to';
        throw new SessionResolutionError(`Refusing to write: ${how} (${shortId(session.id)}, ${session.tty}) is ${session.kind}: ${session.reason}.\n` +
            ...
            formatSessionList(sessions, this.pinnedId), sessions);
    }
}
function pickLocalShell(sessions) {
...
export function formatSessionList(sessions, pinnedId) {
    if (sessions.length === 0)
        return 'iTerm sessions (0)';
